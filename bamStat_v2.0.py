# -*- coding: utf-8 -*-
"""
Created on Mon Apr 24 10:50:26 2017
...
#	
#	structure modification : 2017 04 24
#	modify for accepting single file input
#############################################
...
def main():
	...
	if options.pmode == 0:
		...
		for rawBamFile in rawBamFiles:
			mainPool.apply_async(mainExc,args=(rawBamFile,bedDct,options.sortedDir,options.resDir,options.pluginDir))
		mainPool.close()
		mainPool.join()
	elif options.pmode == 1:
		mainExc(options.tgtBam,bedDct,options.sortedDir,options.resDir,options.pluginDir)
...
def mainExc(bamFile,bedDct,rawDir,resDir,pluginDir):
	rawName=re.findall(r'(IonXpress\_\d+\_rawlib)',bamFile)[0]
	checkPreFiles(rawName,rawDir,resDir,pluginDir)
	processingSam(rawName,rawDir,resDir,bedDct,pluginDir)
...
def checkPreFiles(rawName,rawDir,resDir,pluginDir):
	'''
	...
	if not os.path.exists(sortedBam):
		...
		if not os.path.exists(mappedBam):
			rawBam=mappedBam+'.basecaller.bam'
			mapBam(rawDir,rawBam,pluginDir)
		sortBam(rawDir,mappedBam,pluginDir)
	getSam(rawDir,resDir,sortedBam,pluginDir)
